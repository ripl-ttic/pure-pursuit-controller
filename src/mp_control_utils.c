...
// Initial attempt at skid-steer PP controller
int
mp_control_compute_control_deltav_via_target_pose (mp_control_utils_t *self, double *deltav)
{


    double la = 0.1; // This should be defined in the config file
    double xa = self->bot_states->x + la*cos(self->bot_states->yaw);
    double ya = self->bot_states->y + la*sin(self->bot_states->yaw);

    double L1 = sqrt ((self->target_states->x - xa)*(self->target_states->x - xa) + (self->target_states->y - ya)*(self->target_states->y - ya));

    double eta = atan2(ya - self->target_states->y, xa - self->target_states->x) + self->bot_states->yaw;

    double wheelbase = 0.535;
    *deltav = -1 * (wheelbase * sin(eta))/(L1/2 + la*cos(eta));
