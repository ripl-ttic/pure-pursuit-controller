...
obs_rect_t* mp_prediction_check_collision_with_rect_list (mp_prediction_t *self, GSList *states, GSList *inputs,
    ...
    obs_rect_t *colliding_rect = NULL;
    ...
    while (states_ptr) {
        ...
        for (int i = 0; i < rect_list->num_rects; i++) {
            obs_rect_t *rect_curr =  &(rect_list->rects[i]);
            ...
            if (absolute_distance < max_rect_size + VEHICLE_SECURITY_RADIUS) {
                colliding_rect = obs_rect_t_copy (rect_curr);
...
int mp_prediction_draw_colliding_rect (mp_prediction_t *self, obs_rect_t *colliding_rect) {
